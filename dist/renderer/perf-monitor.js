...
exports.updateActiveFeatures = updateActiveFeatures;
exports.show = show;
exports.hide = hide;
...
function createPanel() {
  ...
  `;
    const copyBtn = panel.querySelector('.perf-copy');
    copyBtn.addEventListener('click', () => {
        copySnapshot(copyBtn);
    });
    const closeBtn = panel.querySelector('.perf-close');
...
function generateWarnings() {
    ...
    if (pipelineState.isRecording) {
        const avgMs = pipelineState.frameCount > 0 ? pipelineState.profile.total / pipelineState.frameCount : 0;
        if (avgMs > DANGER_FRAME_BUDGET_MS) {
            w.push(`Frame time ${avgMs.toFixed(1)}ms exceeds 50ms budget — recording will drop frames. Disable cinema filters or reduce active overlays.`);
        }
        else if (avgMs > WARNING_FRAME_BUDGET_MS) {
            w.push(`Frame time ${avgMs.toFixed(1)}ms is over 33ms budget — consider disabling film grain or reducing active overlays.`);
        }
        if (pipelineState.droppedFrames > 5) {
            w.push(`${pipelineState.droppedFrames} frames dropped in last window — rendering can't keep up with ${pipelineState.targetFps}fps target.`);
        }
        ...
        if (pipelineState.frameCount > 0) {
            ...
            if (cinemaAvg > 10) {
                w.push(`Cinema filter averaging ${cinemaAvg.toFixed(1)}ms/frame — film grain and vignette are GPU-intensive. Try "none" filter.`);
            }
            if (screenAvg > 15) {
                w.push(`Screen capture drawing ${screenAvg.toFixed(1)}ms/frame — zoom crop is expensive. Reduce zoom usage.`);
            }
            if (cameraAvg > 10) {
                w.push(`Camera drawing ${cameraAvg.toFixed(1)}ms/frame — check camera resolution (ideal: 720p for recording).`);
            }
            if (socialsAvg > 5) {
                w.push(`Social overlays ${socialsAvg.toFixed(1)}ms/frame — reduce social links count or disable them.`);
            }
        }
    }
    // Memory warnings
    ...
    if (highCostFeatures.length >= 3) {
        w.push(`${highCostFeatures.length} high-cost features active simultaneously. Consider disabling some for smoother recording.`);
    }
    return w;
}
function renderPanel() {
    ...
    // Preview FPS
    const previewFpsEl = panelEl.querySelector('#perf-preview-fps .perf-fps-value');
    ...
    // Recording FPS
    const recFpsEl = panelEl.querySelector('#perf-rec-fps .perf-fps-value');
    if (pipelineState.isRecording) {
    ...
    // Dropped frames
    const droppedEl = panelEl.querySelector('.perf-dropped-value');
    ...
    // Memory
    const memBar = panelEl.querySelector('#perf-memory-bar');
    const memText = panelEl.querySelector('#perf-memory-text');
    if (heapLimitMB > 0) {
    ...
    // Active features
    const featList = panelEl.querySelector('#perf-features-list');
    featList.innerHTML = '';
    ...
    // Warnings
    const warningsSection = panelEl.querySelector('#perf-warnings-section');
    const warningsList = panelEl.querySelector('#perf-warnings-list');
    ...
    // Recorder info
    const recInfo = panelEl.querySelector('#perf-recorder-info');
    if (pipelineState.isRecording) {
...
function renderFrameBreakdown() {
    ...
    const bar = panelEl.querySelector('#perf-frame-bar');
    const legend = panelEl.querySelector('#perf-frame-legend');
    const avgLabel = panelEl.querySelector('#perf-frame-avg');
    if (!pipelineState.isRecording || pipelineState.frameCount === 0) {
    ...
    for (const section of sections) {
        ...
        seg.style.width = `${pct}%`;
        seg.style.backgroundColor = SECTION_COLORS[section] ?? '#888';
        seg.title = `${section}: ${(value / n).toFixed(1)}ms (${pct.toFixed(0)}%)`;
        bar.appendChild(seg);
    }
    // Budget marker
    const budgetPct = (WARNING_FRAME_BUDGET_MS / Math.max(avgTotal, WARNING_FRAME_BUDGET_MS * 1.2)) * 100;
    ...
    for (const section of sections) {
        ...
        dot.style.backgroundColor = SECTION_COLORS[section] ?? '#888';
...
function formatSnapshot() {
    ...
    lines.push(`  Recording FPS: ${pipelineState.isRecording ? String(currentRecFps) : 'N/A (not recording)'}`);
    ...
    if (pipelineState.isRecording && pipelineState.frameCount > 0) {
        const n = pipelineState.frameCount;
        const avgTotal = pipelineState.profile.total / n;
        lines.push(`  Avg total: ${avgTotal.toFixed(1)}ms (budget: ${WARNING_FRAME_BUDGET_MS.toFixed(1)}ms)`);
        const sections = [
            'setup',
            'screen',
            'camera',
    ...
    if (heapLimitMB > 0) {
        ...
        lines.push(`  Heap used:  ${heapUsedMB.toFixed(0)}MB / ${heapTotalMB.toFixed(0)}MB (${pct}% of limit)`);
