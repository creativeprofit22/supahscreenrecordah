...
// src/preload/edit-modal-preload.ts
...
var editModalAPI = {
  close: () => {
    import_electron.ipcRenderer.send(Channels.EDIT_MODAL_CLOSE);
  },
  save: (data) => {
    import_electron.ipcRenderer.send(Channels.EDIT_MODAL_SAVE, data);
  },
  previewOverlay: (data) => {
    import_electron.ipcRenderer.send(Channels.OVERLAY_PREVIEW, data);
  },
  testCta: () => {
    import_electron.ipcRenderer.send(Channels.CTA_TEST);
  },
  getConfig: () => import_electron.ipcRenderer.invoke(Channels.CONFIG_GET)
};
import_electron.contextBridge.exposeInMainWorld("editModalAPI", editModalAPI);
//# sourceMappingURL=edit-modal-preload.js.map
